...
class HorarioModel:
  ...
  def agregar_evento(
    ...
    """Inserta un evento manual o reserva en un espacio."""
    conn = sqlite3.connect(self.db_path)
    cursor = conn.cursor()
    ...
    # Insertar el nuevo bloque ocupado
    cursor.execute(
        """
            INSERT INTO bloques (espacio, dia, hora_inicio, hora_fin, duracion_horas, franja_horaria, asignatura, docente, estado)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'OCUPADO')
        """,
        (
            espacio,
  ...
  def eliminar_evento(self, record_id):
    """Elimina un evento registrado por su ID."""
    conn = sqlite3.connect(self.db_path)
    cursor = conn.cursor()
  ...
  def actualizar_evento(
    ...
    """Actualiza la información de un bloque o evento."""
    conn = sqlite3.connect(self.db_path)
    cursor = conn.cursor()
    duracion = hora_fin - hora_inicio
    franja = f'{hora_inicio}:00 - {hora_fin}:00'
    ...
    cursor.execute(
        """
            UPDATE bloques
            SET espacio=?, dia=?, hora_inicio=?, hora_fin=?, duracion_horas=?, franja_horaria=?, asignatura=?, docente=?, estado='OCUPADO'
            WHERE rowid=?
        """,
        (
            espacio,
