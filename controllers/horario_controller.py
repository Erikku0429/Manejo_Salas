...
def formatear_12h(hora_24):
    """Convierte un entero (7 a 19) a string en formato 12 horas (ej. 7 -> 07:00 AM, 14 -> 02:00 PM)."""
    h = int(hora_24)
    dt = datetime.time(hour=h)
    return dt.strftime("%I:%00 %p").lstrip("0")

def parsear_12h_a_24h(str_12h):
    """Convierte un texto tipo '02:00 PM' a entero 24H (ej. 14)."""
...
if st.session_state.authenticated and tab_edicion:
    with tab_edicion:
        ...
        if df_db.empty:
            ...
            # Preparar datos: convertir horas 24h a 12h para mostrar al usuario sin la columna ID
            ...
            if st.button("💾 Guardar Cambios en la Base de Datos", type="primary"):
                try:
                    # Convertir horas 12H a 24H internamente para procesar
